...
/**
 * Procesa automáticamente todos los mensajes sin etiqueta "processed"
 * Este endpoint es llamado por Cloud Scheduler cada 15 minutos
 * Procesa mensajes de las últimas 24 horas para no procesar mensajes muy antiguos
 */
app.post("/control/process-unprocessed", async (req, res) => {
  try {
    ...
    if (serviceStatus.status === "stopped") {
      console.log("[mfs] /control/process-unprocessed → Servicio detenido. Saltando procesamiento automático.");
      return res.status(200).json({
        ok: true,
        message: "Servicio detenido. No se procesaron mensajes.",
        status: "stopped",
      });
    }
    
    // Procesar mensajes de las últimas 24 horas (para no procesar mensajes muy antiguos)
    const hoursAgo = 24;
    const secondsAgo = hoursAgo * 60 * 60;
    const timestampAgo = Math.floor(Date.now() / 1000) - secondsAgo;
    
    console.log(`[mfs] /control/process-unprocessed → Procesamiento automático: mensajes sin processed de las últimas ${hoursAgo} horas`);
    console.log(`[mfs] /control/process-unprocessed → Timestamp desde: ${timestampAgo} (hace ${hoursAgo} horas)`);
    ...
    try {
      ...
      // Query: INBOX, sin etiqueta processed, últimas 24 horas
    ...
    res.json({
      ok: true,
      message: `Procesamiento automático completado: ${totalProcesados} procesados, ${totalFallidos} fallidos, ${totalSaltados} saltados`,
      totalEncontrados,
