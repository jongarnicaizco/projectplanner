# Script para desplegar el fix de from === to
...
Write-Host "DESPLEGANDO FIX: FROM === TO" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($status) {
    git commit -m "Fix: Priorizar BCC y mailing list cuando from === to" 2>&1 | Write-Host
...
$imageTag = "fix-from-equals-to-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "El servicio ahora priorizará BCC/mailing list cuando from === to." -ForegroundColor Cyan
} else {
    Write-Host ""
