# Script para desplegar el fix de mailing list cuando To no tiene dominio válido
...
Write-Host "DESPLEGANDO FIX: MAILING LIST PRIORITY" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($status) {
    git commit -m "Fix: Buscar mailing list cuando To no tiene dominio válido" 2>&1 | Write-Host
...
$imageTag = "fix-mailing-list-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "El servicio ahora buscará mailing list cuando To no tiene dominio válido." -ForegroundColor Cyan
} else {
    Write-Host ""
