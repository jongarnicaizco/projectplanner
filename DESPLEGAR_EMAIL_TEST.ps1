# Script para desplegar la funcionalidad de email de prueba
...
if ($status) {
    git commit -m "Add: Enviar email de prueba antes de crear registro en Airtable" 2>&1 | Write-Host
...
$imageTag = "add-email-test-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "El servicio ahora enviará un email de prueba antes de crear cada registro en Airtable." -ForegroundColor Cyan
} else {
    Write-Host ""
